...
#define BED_EMPTY_CHECK_COUNT						2
...
void BedManagerPnpSet(bool bedStt){
	...
	countOccBed = 0;
	pnpBedMovFlag = false;
	LOG_WARN("preBedPnp :  %d  bedStt : %d", bedPnp, bedStt);
	OperationModeCf_t OperationModeCf = SettingGetModeOperation();
	if(OperationModeCf.mode == OPE_MODE_AUTO){
	...
	if(preBedPnp == true && bedStt == false){
		EmptyProcess();
	}
...
void BedManagerPnpPredict(BedManagerPnpSate_t BedManagerPnpSate)
{
	...
	//		LOG_WARN("pnpBedMovFlag: %d", pnpBedMovFlag);
	//		return;
	//	}
	...
	else if(BedManagerPnpSate == BED_MANAGER_PNP_BED_GOOD)
	{
