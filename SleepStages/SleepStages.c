...
void SleepStagesProcess()
{
	if((SleepStagesParseData() == SLEEP_STATUS_OK) && (dataIndex >= 5)){
		SleepStagesCal();
...
void SleepStagesUpdateLcd(){
	...
	sleepData.timeStart = sleepStageOutput->unixTime[0] + (SettingGetTimeZone() * 3600);
	sleepData.timeEnd = sleepStageOutput->unixTime[dataSize-1] + (SettingGetTimeZone() * 3600);
