...
void user_main_mode(void)
{
	...
	if(Qstatus == osOK)
	{
		...
		{
			if(hrRr2Data.type == EMPTY_DATA && LcdCtrlIsSleeping() == true){
		...
		if(hrRr2Data.type == HEART_RATE_DATA || hrRr2Data.type == EMPTY_DATA)
		{
			...
			/*Show motions*/
			LcdUpdateMotion((uint32_t)hrRr2Data.lavie_data.hr_rr.motions);
...
static void (*display_state_table[4])(void) = {
		user_main_mode,
		user_calibration_mode,
};
