...
bool start_calib(void)
{
	LOG_DEBUGF((char*)"-----------Start calibration mode---------");
	CalibPrepare();
	...
	{
		LcdUpdateTextCalibrationPopUp(msgCalibEbDone);
		...
		if(calib_person(&calib_params) == true){
			successFlag = true;
			save_calib_params(calib_params);
		}
	}

	if(successFlag == true && deviceIsCalibrated() == true){
