...
#define TIME_10MINUTES (uint32_t)600000
...
static uint32_t timeCheckBodyTemp = TIME_10MINUTES;//5 minutes
	...
	for(;;)
	{
		...
		if (Qstatus == osOK)
		{
			...
			if(pnp_bg_index < ACTUAL_DATA_LEN)
			{
				...
				if(flagResumPnp){
					osThreadResume(pnpBGTaskHandle);
					flagResumPnp = false;
				}
			}
			...
			if(BedManagerPnpGet() || (SettingGetModeOperation().mode == OPE_MODE_MANUAL))
			{
				...
				{
					resplife_index = 0;
					if((SettingGetModeOperation().mode == OPE_MODE_AUTO) && (lavie_data_combine.bed_occupied == true)){
...
//					if(SettingGetModeOperation().mode == OPE_MODE_MANUAL){
//						lavie_data_combine.hr_rr.apnea = 0;
//					}
//					else{
...
//					}
					...
					{
//						if(SettingGetModeOperation().mode == OPE_MODE_MANUAL /*&& lavie_data.hr_rr.actual_hr != PREVIOUS_CASE*/ /*&& lavie_data.hr_rr.motions == 0*/)
//						{
//						if(lavie_data.hr_rr.motions == 0)
...
//							LOG_WARN((char*)"holdHr : %d",holdHr);
					...
					{
						holdHr = 0;
						if(BedManagerGetPnpState() == BED_MANAGER_PNP_BED_GOOD){
							lavie_data_combine.bed_occupied = true;
							...
							LOG_WARN((char*)"BedManagerGetPnpState != BED_MANAGER_PNP_BED_GOOD");
							HrRr2Display(HEART_RATE_DATA, &lavie_data_combine);
							HrRr2Display(RESP_RATE_DATA, &lavie_data_combine);
							publish_hrrr_data(lavie_data_combine);
						}
				...
				if(resp_index >= RESP_RAW_DATA_LEN) // enough RESP_RAW_DATA_LEN samples -> find respiration
				{
					resp_index = 0;
					memset(&lavie_data, 0, sizeof(lavie_data));
					resp_detect(&resp_data, RESP_RAW_DATA_LEN, &lavie_data.hr_rr.rr);
...
void OccupiedProcess(bool startFlag)
{
	...
	timeCheckBodyTemp = TIME_10MINUTES;
	timeBaseCheckBodyTemp = osKernelGetTickCount();
