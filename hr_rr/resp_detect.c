...
void resp_detect(resp_data_t *resp_data, const uint16_t data_size, uint8_t *RR)
{		
	 *RR = PREVIOUS_CASE;
...
//			LOG_DEBUG("is_inhale");
		...
		if(time_apnea_sec >= 20.0)
		{
			*is_apnea = true;
			if(time_apnea_sec < 30){
				countApnea++;
				countApnea40s++;
			}
...
resp_status_t lifeline_resp(resplife_data_t* resplife_data, const uint16_t data_size)
{
    ...
    if(size_resp > 2){
		...
		if(num_mov_lifeline == 0)
		{
			...
			if(get_nobreathing_setting() == NOBREATH_DISABLE)
			{
